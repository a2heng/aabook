...
- **没有提示语**的台词（前后都没写谁说的）：在引号前**补一句** `edit(op="replace", find="“开头几字", replace="XXX说道：“开头几字")`；动词随语境选：说/问/答/回应/喊道/低声道/笑道，别一律"说"。
...
✓ 无提示语：`“我们走吧。”` → 先 `replace(find="“我们走吧", replace="琥珀说道：“我们走吧")` 再 speak
✓ 长台词分多次 speak，程序自动拼合
✓ `“你……别过来！！！”` → replace `……`→`，`、`！！！`→`！`
