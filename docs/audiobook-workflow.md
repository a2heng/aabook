...
- 正文里**每句前后都有唯一的切割标记**，形如 **`[3A]`、`[22D]`（阿拉伯数字段落号+大写字母）**，
  相邻句共用中间的标记、行尾有收尾标记；标题也算段。
- `edit(op="speak", begin="3B", end="3C", text="这段原文", role="规范名")`：**begin/end/text/role 四个参数缺一不可**（提示词铁律 + schema `required` 全必填）；begin/end 是这段发言前后两个标记。
- **MCP 宽容修**：标签容忍 `[3B]`/`3b`/`3-B`、end 可省、`line` 可由标记推出、跨段按起始段解；span 自动裁到引号内；**跨多引号自动拆成「每段一个标记」**（旁白留在标记外）；半截引号自动配对；span 内旧标记摊平防套娃；无引号 span 必须有「说道/想」类提示，否则按旁白拒绝；**原文一个字不改**（不清理引号、不加气口）。
- `edit(op="unmark", …)` 去掉误标（只去标记、不动原文）；同一处再 `speak` 会覆盖旧标记并支持收紧范围。
- **1 次标注 + 1 轮检查（`CHECK_ROUNDS=1`，`--check-steps` 默认 100，0=关闭）**：标完机械扫描
